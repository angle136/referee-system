...
{
  ArmorProtocolTxPacket_t queued = {0};
  uint32_t primask;
  bool pushed;

  if (data == NULL || length == 0U || length > ARMOR_ADC_DEBUG_FRAME_SIZE)
  {
    return false;
  }
  ...
  __disable_irq();
  pushed = Kfifo_Push(&armor_tx_fifo, &queued);
                              ...
                              uint8_t reset_sequence)
{
  ...
  packet[7] = sum;
  (void)ArmorProtocol_Queue(packet, ARMOR_PACKET_SIZE);
}

void ArmorProtocol_SendAdcDebug(const uint16_t *samples)
{
  uint8_t packet[ARMOR_ADC_DEBUG_FRAME_SIZE];
  uint8_t sum = 0U;
  ...
  packet[0] = 0xA5U;
  packet[1] = ARMOR_ADC_DEBUG_CMD;
  for (uint8_t index = 0U; index < ARMOR_ADC_HISTORY_COUNT; index++)
  {
    packet[2U + index * 2U] = (uint8_t)(samples[index] & 0xFFU);
    packet[3U + index * 2U] = (uint8_t)(samples[index] >> 8U);
  }
